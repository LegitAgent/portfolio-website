...
function WrongPage() {
  ...
  const startDragging = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const viewportInsetX = Math.max(4, Math.min(24, (window.innerWidth - astronautBounds.width) / 2));
    const viewportInsetY = Math.max(4, Math.min(24, (window.innerHeight - astronautBounds.height) / 2));
    ...
    dragStart.current = {
      ...
      offsetX: position.x,
      offsetY: position.y,
      minX: position.x + viewportInsetX - astronautBounds.left,
      maxX: position.x + window.innerWidth - viewportInsetX - astronautBounds.right,
      minY: position.y + viewportInsetY - astronautBounds.top,
      maxY: position.y + window.innerHeight - viewportInsetY - astronautBounds.bottom,
    };
    setIsDragging(true);
  };

  const dragAstronaut = (event: PointerEvent<HTMLDivElement>) => {
    ...
    setPosition({
      x: Math.min(dragStart.current.maxX, Math.max(dragStart.current.minX, nextX)),
      y: Math.min(dragStart.current.maxY, Math.max(dragStart.current.minY, nextY)),
    });
  };
  ...
  return (
    <section className='wrong'>
      <div className='wrong__scene' aria-hidden='true'>
        <div
          className={`astronautDrag${isDragging ? ' is-dragging' : ''}`}
          ref={dragRef}
          style={{ transform: `translate3d(${position.x}px, ${position.y}px, 0)` }}
          onPointerDown={startDragging}
          onPointerMove={dragAstronaut}
          onPointerUp={stopDragging}
          onPointerCancel={stopDragging}
          onDoubleClick={() => setPosition({ x: 0, y: 0 })}
        >
          <svg className='astronaut' viewBox='0 0 128 128'>
        ...
        <span className='spaceBit spaceBit--one'>HTTP 404</span>
        <span className='spaceBit spaceBit--two'>ROUTE_NOT_FOUND</span>
        <span className='spaceBit spaceBit--three'>PATH: {location.pathname}</span>
      </div>

      <div className='wrong__content'>
        <p className='wrong__eyebrow'>404 / route not found</p>
        <h1>Lost in orbit.</h1>
